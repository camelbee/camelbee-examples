...
paths:
  ...
  - "src/main/resources/logback-quarkus.properties"
...
Spring profiles switch format:

| Profile | Appender | Format |
|---|---|---|
| `dev`, `test`, `itest` | Console | Human-readable with color |
| `prod` | Console + Logstash JSON | Structured JSON for log aggregation (ELK, Loki, Datadog) |

Per-logger levels:
- `io.camelbee` — DEBUG in dev, INFO in prod
- `org.apache.camel` — DEBUG in dev, WARN in prod
- `root` — INFO in dev, WARN in prod

### Adding a new log field

Add to `%mdc{newField}` in the pattern, and set via `exchange.getIn().setHeader("newField", value)` + MDC.put("newField", value) in a processor. Don't skip the MDC.put — Logback reads MDC, not headers.
...
| MCP | `McpErrorResponseFormatter` | MCP error response per the MCP spec |
