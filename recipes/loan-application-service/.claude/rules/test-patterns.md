...
CamelBee testing conventions for **SPRINGBOOT** microservices. Three test levels: Unit, Integration, Black-box.
...
public abstract class UnitTest {
    @Autowired protected FluentProducerTemplate fluentProducerTemplate;
...
| Kafka | `bbtest/kafka/` | KafkaProducer + KafkaConsumer |
...
| 2 | `src/main/resources/application.yml` + `application-quarkus.yml` | What the **app** sends when connecting to the backend | App JVM (or Docker when bbtest runs) |
...
|---|---|---|---|---|---|
| PostgreSQL | user/password/db | `POSTGRES_USER`, `POSTGRES_PASSWORD`, `POSTGRES_DB` | `spring.datasource.*` / `quarkus.datasource.*` | `POSTGRES_*` | `DataSeeder.executeSqlScript(jdbc:postgresql://localhost:5432/CAMELBEE_DATABASE, camelbee_user, secret)` + `DataVerifier.SQL_USER` |
| Redis/Valkey (CACHE) | host/port (typically no auth in dev) | `cache:` service on 6379 | `cache.redis.host/port` | `CACHE_REDIS_HOST`, `CACHE_REDIS_PORT` | `Jedis("localhost", 6379)` / equivalent in `DataVerifier` |
...
|---|---|---|
| Kafka | `kafka:{{topic}}-{format}` | Binary formats need ByteArraySerializer |
