...
|---|---|---|---|
| MongoDB | `src/integration-test/resources/docker/mongodb/init-mongo.js` | `src/integration-test/resources/backend/mongodb/reset-mongodb.json` | Integration + BB |
...
- `resetMongodbData()` in `IntegrationTest.java` / `DataSeeder.java`
- Reads `reset-mongodb.json` (BSON extended JSON) and `deleteMany({}) + insertMany(...)`
- Capped `order_events` collection auto-evicts — for that one, `deleteMany` is often unnecessary (but keep it for determinism)
