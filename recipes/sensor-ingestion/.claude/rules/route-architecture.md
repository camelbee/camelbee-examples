...
| Element | Standard |
|---------|----------|
| Package base | `io.iot.sensor.ingestion` |
| Framework | **QUARKUS** |
...
**Interfaces (Consumer Routes):** MQTT,REST

**Backends (Producer Routes):** KAFKA,MONGODB
...
+-- infra/          <-- Infrastructure models (backend-specific)
    ...
    +-- mongodb/    Purchase.java document
```
...
**Contract-first — the `rest().openApi()` binding is mandatory.** Never hand-define REST endpoints (`rest().get(...)` / `rest().post(...)`); the spec is the single source of truth and the generated models, docs, and test clients all derive from it. API changes go into the OpenAPI spec YAML (then `/cb-regen`), and each `operationId` maps to a `from("direct:{operationId}")` route. The spec file may be renamed (`order-api.yaml` → `<your-domain>-api.yaml`) — update the path in the binding and in the pom.xml `<inputSpec>`; only the mechanism is fixed.
...
from("direct:createOrder")
    ...
    .unmarshal(orderFormat)  // JacksonDataFormat with injected ObjectMapper
    ...
    .marshal(jsonOrderFormat)  // JacksonDataFormat with injected ObjectMapper
    .process(this::setOriginalContentTypePropertiesBack)
    .setHeader(HTTP_RESPONSE_CODE, constant(201));
```

### MQTT Consumer (Interface)

```java
from("paho-mqtt5:{{${serviceName}.northbound-createorder-topic}}-json?brokerUrl={{${serviceName}.mqtt.broker-url}}")
...
Topic naming: `${serviceName}.northbound-{operation}-topic-{format}`. Uses MQTTv5 with `paho-mqtt5` component.
...
Architecture-level shape per backend; full DSL examples for every backend are in `camel-dsl-patterns.md`.

### NoSQL Backends (Cassandra / MongoDB / DynamoDB)

Same shape for all three: restore `ORIGINAL_BODY`, map to the backend's infra model in a processor, call the component, convert back to `Order`, set `ACTUAL_RESPONSE_BODY`. List operations support cursor-based pagination.

| Backend | Endpoint | Notes |
|---|---|---|
| MongoDB | `mongodb:mongoBean?database=...&collection=...&operation=insert` | Body is a BSON `Document` |
