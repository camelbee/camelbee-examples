...
import com.mycompany.catalog.mcp.itest.CreateOrderIntegrationTest;
 ...
 */
  ...
  public class McpInterfaceCreateOrderIntegrationTest extends CreateOrderIntegrationTest {
    ...
    private static final JsonSerDe<Map<String, Object>> REQUEST_SERDE =
    ...
    private void validateMcpOrderFromText(String resultText) {
      try {
        JsonSerDe<com.mycompany.catalog.mcp.model.api.mcp.Order> jsonOrderSerDe =
            new JsonSerDe<>(new TypeReference<>() {});

        com.mycompany.catalog.mcp.model.api.mcp.Order order = jsonOrderSerDe.deserialize(resultText);

          List<String> itemIds = order.getItems().stream()
              .map(com.mycompany.catalog.mcp.model.api.mcp.OrderItem::getId).toList();
