...
| Element | Standard |
|---------|----------|
| Package base | `com.mycompany.catalog.mcp` |
| Framework | **QUARKUS** |
...
NOTE: The archetype-generated producer routes are independent — each reads
...
**These properties are conventions, not rigid rules.** The archetype uses them in a simple pattern where each producer independently reads `ORIGINAL_BODY`. In your microservice, you can adapt the flow: pass data between producers via body or exchange properties, merge responses, chain backends where one's output feeds the next, or skip `ORIGINAL_BODY` entirely if the logic doesn't need it. The only property the consumer route relies on for the final response is `ACTUAL_RESPONSE_BODY`.
