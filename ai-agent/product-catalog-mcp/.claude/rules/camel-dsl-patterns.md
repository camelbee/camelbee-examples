...
When REST supports **multiple formats** (JSON + XML + Proto + Avro), the consumer uses dedicated choice routes instead of inline unmarshal. The decision is made at archetype generation time:
...
from("direct:unmarshalOrder")
    .choice()
        ...
        .when(exchangeProperty(ORIGINAL_CONTENT_TYPE).contains(APPLICATION_XML))
            .unmarshal().jaxb("com.mycompany.catalog.mcp.model.api.xml")
...
from("direct:createOrderJpa").routeId("createOrderJpaRoute")
    ...
    .to("jpa-{{jpaVendor}}:com.mycompany.catalog.mcp.model.infra.jpa.Purchase")
...
Central routes wire backends conditionally based on archetype configuration:
```java
// Only generated if REST backend with CRO operation was selected
.to("direct:createOrderRest").id("createOrderRestEndpoint")
```

This is resolved at archetype generation time, not at runtime.
