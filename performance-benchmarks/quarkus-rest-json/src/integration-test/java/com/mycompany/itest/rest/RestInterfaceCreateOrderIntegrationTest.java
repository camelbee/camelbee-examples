...
public class RestInterfaceCreateOrderIntegrationTest extends CreateOrderIntegrationTest {
  ...
  public void setup() throws Exception {
    ...
    routeDefinition.from("direct:testRestCreateOrder").to(
        "http:localhost:{{quarkus.http.test-port}}" + ORDER_BASE_URL + "?throwExceptionOnFailure=false");
  ...
  private static Stream<Arguments> createOrderSuccessParameters() {
    return Stream.of(
        arguments(RequestScenarios.CREATE_ORDER_SUCCESS, "json", APPLICATION_JSON, Status.CREATED.getStatusCode(), TXID_FOR_SUCCESS)
    );
  }
  ...
  private static Stream<Arguments> createOrderBadRequestFromTheInterfaceParameters() {
    return Stream.of(
        arguments("createorder-interface-invaliddate-error-400-request", "json", APPLICATION_JSON, Status.BAD_REQUEST.getStatusCode(), TXID_FOR_SUCCESS),
        arguments("createorder-interface-invalidjsonbody-error-400-request", "json", APPLICATION_JSON, Status.BAD_REQUEST.getStatusCode(), TXID_FOR_SUCCESS),
        arguments(RequestScenarios.CREATE_ORDER_ERROR_NO_ITEMS, "json", APPLICATION_JSON, Status.BAD_REQUEST.getStatusCode(), TXID_FOR_SUCCESS)
    );
  }
  ...
  private void validateJsonOrder(Exchange result) throws Exception {
    JsonSerDe<com.mycompany.model.api.json.Order> jsonSerDe = new JsonSerDe<>(new TypeReference<com.mycompany.model.api.json.Order>() {
    });

    com.mycompany.model.api.json.Order response = jsonSerDe.deserialize(result.getMessage().getBody(String.class));

    validateOrderCommonAssertions(
        response.getId(),
        response.getOrderDate(),
        response.getLastUpdateTimestamp(),
        response.getItems().stream().map(com.mycompany.model.api.json.OrderItem::getId).toList()
    );
  }
