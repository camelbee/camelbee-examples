...
class RestInterfaceCreateOrderIntegrationTest extends CreateOrderIntegrationTest {
  ...
  private static Stream<Arguments> createOrderSuccessParameters() {
    return Stream.of(
        arguments(RequestScenarios.CREATE_ORDER_SUCCESS, "json", APPLICATION_JSON, HttpStatus.CREATED.value(), TXID_FOR_SUCCESS)
    );
  }
  ...
  private static Stream<Arguments> createOrderBadRequestFromTheInterfaceParameters() {
    return Stream.of(
        arguments("createorder-interface-invaliddate-error-400-request", "json", APPLICATION_JSON, HttpStatus.BAD_REQUEST.value(), TXID_FOR_SUCCESS),
        arguments("createorder-interface-invalidjsonbody-error-400-request", "json", APPLICATION_JSON, HttpStatus.BAD_REQUEST.value(), TXID_FOR_SUCCESS),
        arguments(RequestScenarios.CREATE_ORDER_ERROR_NO_ITEMS, "json", APPLICATION_JSON, HttpStatus.BAD_REQUEST.value(), TXID_FOR_SUCCESS)
    );
  }
