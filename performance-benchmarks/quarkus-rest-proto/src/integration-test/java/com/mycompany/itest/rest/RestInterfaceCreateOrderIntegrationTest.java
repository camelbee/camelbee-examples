...
public class RestInterfaceCreateOrderIntegrationTest extends CreateOrderIntegrationTest {
  ...
  private static Stream<Arguments> createOrderSuccessParameters() {
    return Stream.of(
        arguments(RequestScenarios.CREATE_ORDER_SUCCESS, "proto", APPLICATION_PROTOBUF, Status.CREATED.getStatusCode(), TXID_FOR_SUCCESS)
    );
  }
  ...
  private static Stream<Arguments> createOrderBadRequestFromTheInterfaceParameters() {
    return Stream.of(
        arguments(RequestScenarios.CREATE_ORDER_ERROR_NO_ITEMS, "proto", APPLICATION_PROTOBUF, Status.BAD_REQUEST.getStatusCode(), TXID_FOR_SUCCESS)
    );
  }
