...
**Throughput**: ~16,282 req/s
**Improvement**: +12.1% over Run 1
...
**Throughput**: ~16,381 req/s
**Improvement**: +12.8% over Run 1, +0.6% over Run 2
...
| **Max Latency (ms)** | 1,010 | 315.35 | 123.15 |
| **Total Requests** | 1,751,845 | 1,965,359 | 1,978,186 |
| **Success Rate** | 100% | 100% | 100% |

## Key Observations

1. **JVM Warm-up Effect**: Performance improved by ~12% after initial warm-up, demonstrating effective JIT compilation
2. **High Throughput**: Achieved stable ~16.4K requests/second after warm-up
...
- **CPU Usage**: Peaked at ~200% (fully utilizing both allocated cores) during each test run, dropping to ~0.92% at idle between runs
- **Memory Usage**: 674.3MB / 1GB — stabilized at ~674MB after warm-up, with steady memory usage throughout all test runs
- **Disk Read/Write**: 0B read / 442KB write — minimal disk activity, with small incremental writes (likely logging) accumulating across test runs
- **Network I/O**: 8.52GB received / 10.6GB sent cumulative across all three test runs, reflecting the significantly higher throughput compared to Spring Boot variants
...
5. **Memory Efficiency**: Quarkus JVM demonstrates efficient resource consumption at 674MB for high-throughput workloads
