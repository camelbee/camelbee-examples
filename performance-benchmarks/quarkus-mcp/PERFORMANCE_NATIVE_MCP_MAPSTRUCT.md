...
**Throughput**: ~9,583 req/s
**Improvement**: -1.5% vs Run 1 (consistent with native's stable performance profile — no JIT warm-up benefit)
...
**Improvement**: -0.7% vs Run 1 (native executables maintain consistent performance without JIT optimization)
...
| **Max Latency (ms)** | 199.04 | 231.16 | 177.55 |
| **Total Requests** | 1,178,670 | 1,160,894 | 1,169,782 |
| **Success Rate** | 100% | 100% | 100% |

## Key Observations

1. **Consistent Performance**: Native executables show remarkably stable performance across runs — all three runs within ~1.5% of each other
2. **Stable Throughput**: Achieved ~9.7K requests/second consistently
3. **Predictable Latency**: Median latency around 12ms with very consistent P90/P95 profiles across all runs
4. **Exceptional Memory Efficiency**: Uses only ~188MB memory (within a 256MB container limit)
...
- **CPU Usage**: Peaked at ~200% (fully utilizing both allocated cores) during each test run, dropping to ~0.02% at idle between runs
- **Memory Usage**: 188.2MB / 256MB — stabilized at ~188MB with gradual step increases during the first two runs before leveling off
- **Disk Read/Write**: 83.1MB read / 0B write — disk reads occurred at startup (native binary loading), zero writes during tests
- **Network I/O**: 5.34GB received / 6.82GB sent cumulative across all three test runs
