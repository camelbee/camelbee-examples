...
Build and start the application with all backends using Docker Compose:

```bash
docker compose up --build
```

This starts the microservice with all CamelBee debugging features enabled (notifier, tracer, context, logging) via the environment variables configured in `docker-compose.yml`.
