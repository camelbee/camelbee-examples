...
|-----------|-------|----------------|------|
| REST/MCP API models | `src/main/resources/openapi/order-api.yaml` | `com.mycompany.product.catalog.model.api.json` | REST, MCP, WebSocket, or JSON formats |
| MCP API models | `src/main/resources/mcp/order-mcp-api.yaml` | `com.mycompany.product.catalog.model.api.mcp` | MCP interface |
| REST backend models | `src/main/resources/backends/openapi/purchase-api.yaml` | `com.mycompany.product.catalog.model.infra.json` | REST backend |

**Key config:**
- Generator: `jaxrs-spec` with `useJakartaEe=true`
...
- Component model: `cdi` (generates CDI `@ApplicationScoped` implementations)
...
6. **For Quarkus native:** register new model classes in `config/ReflectionConfig.java`
