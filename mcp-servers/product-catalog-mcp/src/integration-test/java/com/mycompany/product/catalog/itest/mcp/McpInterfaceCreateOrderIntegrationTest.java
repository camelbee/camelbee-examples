...
import com.mycompany.product.catalog.itest.CreateOrderIntegrationTest;
import com.mycompany.product.catalog.itest.IntegrationTestProfile;
import com.mycompany.product.catalog.utils.JsonSerDe;
import com.mycompany.product.catalog.utils.testdata.CreateOrderDomainTestDataProducer.RequestScenarios;
...
public class McpInterfaceCreateOrderIntegrationTest extends CreateOrderIntegrationTest {
  ...
  private void validateMcpOrderFromText(String resultText) {
    try {
      JsonSerDe<com.mycompany.product.catalog.model.api.mcp.Order> jsonOrderSerDe = new JsonSerDe<>(new TypeReference<>() {
      });

      com.mycompany.product.catalog.model.api.mcp.Order order = jsonOrderSerDe.deserialize(resultText);

      List<String> itemIds = order.getItems().stream()
          .map(com.mycompany.product.catalog.model.api.mcp.OrderItem::getId).toList();
