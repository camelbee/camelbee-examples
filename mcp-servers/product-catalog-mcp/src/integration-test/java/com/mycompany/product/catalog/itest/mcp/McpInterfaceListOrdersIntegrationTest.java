...
import com.mycompany.product.catalog.itest.ListOrdersIntegrationTest;
...
public class McpInterfaceListOrdersIntegrationTest extends ListOrdersIntegrationTest {
  ...
  private void validateMcpOrdersFromText(String resultText, int expectedOrder) {
    try {
      JsonSerDe<List<com.mycompany.product.catalog.model.api.mcp.Order>> jsonOrdersSerDe = new JsonSerDe<>(new TypeReference<>() {
      });

      List<com.mycompany.product.catalog.model.api.mcp.Order> orders = jsonOrdersSerDe.deserialize(resultText);
