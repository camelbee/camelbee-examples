...
public interface JsonInfraErrorMapper {
  ...
  Error jsonErrorToDomainError(com.mycompany.product.catalog.model.infra.json.Error error);

  // Domain Error to Json Error
  @Mapping(source = "code", target = "code")
  @Mapping(source = "message", target = "message")
  com.mycompany.product.catalog.model.infra.json.Error domainErrorToJsonError(Error error);
